...
SwapChain::SwapChain(::xrn::engine::vulkan::Device &deviceRef, VkExtent2D extent)
    : device{deviceRef}, windowExtent{extent} {
    this->init();
}

SwapChain::SwapChain(::xrn::engine::vulkan::Device &deviceRef, VkExtent2D extent, ::std::shared_ptr<::xrn::engine::vulkan::SwapChain> previous)
...
void SwapChain::createFramebuffers() {
  ...
  for (size_t i = 0; i < imageCount(); i++) {
    ...
    VkExtent2D vkSwapChainExtent = getSwapChainExtent();
...
VkPresentModeKHR SwapChain::chooseSwapPresentMode(
  ...
  for (const auto &availablePresentMode : availablePresentModes) {
    if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
      XRN_DEBUG("Present mode: Mailbox");
  ...
  for (const auto &availablePresentMode : availablePresentModes) {
    if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) {
      XRN_DEBUG("Present mode: Immediate");
      return availablePresentMode;
    }
  }

  XRN_DEBUG("Present mode: V-Sync");
